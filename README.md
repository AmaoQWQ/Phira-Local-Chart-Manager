...
| 文件 | 用途 | 是否可以提交 |
| --- | --- | --- |
| `certs/server.crt` | 服务器证书，相当于公开的服务器身份证 | 本项目仍默认忽略，部署时单独提供 |
| `certs/server.key` | 与证书配套的私钥，用于证明服务器身份并建立加密连接 | **绝对不能提交、公开或发送给他人** |

`npm run cert:generate` 生成的是供本地测试和 Phira 不安全模式使用的自签名证书。自签名只表示证书由自己签发，不代表系统或浏览器已经信任它；直接访问网关的 `8443` 端口时可能出现证书警告。生产环境应使用反向代理或受信任 CA 签发的证书。公网 `443` 入口可能由反向代理提供另一张证书，它与网关直接加载的 `certs/server.crt` 不一定相同。

真实证书和私钥均已由 `.gitignore` 排除。即使仓库是私有仓库，也不要强制添加 `server.key`。如果私钥疑似泄露，应立即更换私钥和对应证书，而不是只删除 Git 中的文件。
...
这里的 `-k` 仅用于忽略本地自签名证书的信任错误，不应作为生产客户端关闭证书校验的常规方案。
