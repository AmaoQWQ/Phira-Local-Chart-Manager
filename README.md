...
自动验证客户端上传的私有成绩需要兼容的 Decoder 和本地验证材料。验证密钥默认读取 `decoder/record-verification-key.bin`，可用 `.env` 中的 `PRIVATE_RECORD_VERIFICATION_KEY_PATH` 指向其他位置。该文件的格式是**原始二进制**，长度必须正好为 **26 字节**；长度不符或文件不存在时会被视为未提供密钥，Gateway 仍可启动，但不会把无法验证的上传当作可信成绩保存。公开仓库不会附带这个文件（`.gitignore` 已排除 `decoder/*.bin`），部署时需要自行放入并妥善保管。管理员仍可在管理页面手动录入本地成绩。
