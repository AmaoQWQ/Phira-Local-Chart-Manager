...
exports.decodePhiraRecordToken = decodePhiraRecordToken;
const node_crypto_1 = require("node:crypto");
const node_zlib_1 = require("node:zlib");
/** Read-only v3 parser. valid does not assert official server acceptance. */
function decodePhiraRecordToken(token, options) {
    let stage = 'envelope';
    ...
    try {
        ...
        const inflated = (0, node_zlib_1.inflateRawSync)(raw, { maxOutputLength: 1048576, info: true });
        if (inflated.engine.bytesWritten !== raw.length)
            throw new Error('Trailing DEFLATE data');
        ...
        const expected = (0, node_crypto_1.createHmac)('sha256', options.key).update(protectedData).digest();
        if (!(0, node_crypto_1.timingSafeEqual)(expected, tag))
            throw new Error('HMAC-SHA256 mismatch');
        ...
        for (let start = 0; start < protectedData.length; start += 16) {
            ...
            for (const stride of [8, 4, 2, 1]) {
                for (let i = 0; i < 16; i++) {
                    if (i & stride)
                        block[i] = (block[i] - block[i - stride]) & 255;
                }
            }
        ...
        if (p.length !== 92 || p[0] !== 3)
            throw new Error('Unsupported record schema');
        ...
        if (!numOfNotes || maxCombo > numOfNotes || perfect + good > numOfNotes ||
            !Number.isFinite(statisticF64At9))
            throw new Error('Invalid record numeric fields');
        ...
        return { valid: true, verification: {
            ...
            payloadSha256: (0, node_crypto_1.createHash)('sha256').update(payload).digest('hex') };
    }
    catch (error) {
        return { valid: false, stage, integrityVerified,
